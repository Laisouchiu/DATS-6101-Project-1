...
round(prop.table(table(bike$member_casual, bike$rideable_type))*100, 2)
```
...
#ask about the statement of p-value too.
ggplot(bike, aes(hour))+
  geom_line(stat = "bin", bins = 30)
summary(bike$hour)
```
**Is there a correlation between time and day of usage?**
```{r}
aov(hour ~ days, data = bike)
```
