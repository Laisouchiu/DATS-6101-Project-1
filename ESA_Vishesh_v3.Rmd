...
ggplot(data = df_rt , mapping = aes(x = defult2, y = test_y)) +
  geom_tile(aes(fill = Freq), colour = "white") +
  geom_text(aes(label = sprintf("%1.0f", Freq)), vjust = 1) +
  scale_fill_gradient(low = "#98c1d9", high = "#8f2d56") +
  theme_bw() +
...
knn.5 <- knn(train=Rt_train_y, test=Rt_test_X, cl=Rt_train_y, k=5)
ACC.5=100 * sum(Rt_test_X == knn.5)/NROW(Rt_test_X)
paste0("Accuracy = ", ACC.5)
print(confusionMatrix(knn.5, as.factor(Rt_test_X) ))


ResultDf = data.frame( k=numeric(0), Total.Accuracy= numeric(0), row.names = NULL )

for (kval in seq(11,50,2)) {
  nasa_pred <- knn(train = Rt_train_y, test = Rt_test_X, cl=Rt_train_y, k=kval)
  #NASA_PREDCross = CrossTable(nasa_test.labels, nasa_pred, prop.chisq = FALSE)
  cm = confusionMatrix(nasa_pred, as.factor(Rt_test_X) ) # from caret library
  cmaccu = cm$overall['Accuracy']
  cmt = data.frame(k=kval, Total.Accuracy = cmaccu, row.names = NULL ) # initialize a row of the metrics 
  ResultDf = rbind(ResultDf, cmt)
}
