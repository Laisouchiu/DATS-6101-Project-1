...
#Remove docked bikes
df_check <-subset(df, rideable_type != 2)
...
```
```{r}
df_check%>%group_by(rideable_type)%>%
  summarise(count = n())

df_check %>% count(rideable_type)
ggplot(df_check, aes(rideable_type, fill = rideable_type))+
  geom_bar()+
  scale_fill_brewer(palette = "BuPu")+
  guides(fill="none")+
  labs(title = "User bike types", x= "bike types")+
...
chisq.test(Chi_rt1)
Chi_rt2 = xtabs(~ rideable_type + Total_Time, data = df_rt)
chisq.test(Chi_rt2)
...
chisq.test(Chi_rt5)
Chi_rt6 = xtabs(~ rideable_type + endingm , data = df_rt)
chisq.test(Chi_rt6)
```
